...
#### 实现细节
除非另有说明。否则我们的用于特征提取的网络由两个卷积层组成。每一层采用32过滤器，每个有一个3x3的内核。每个卷积层后面都有一个修正的线性单元（ReLU）和一个2X2的最大池化层。我们添加了一个跳转连接，并将来自第一卷积层中央作物的激活与第二池层的输出连接起来，以保留更多相关的本地化信息，否则使用max池化层会丢失这些信息。由于收敛所需的时间步长相对较小，我们发现使用维数为512的状态向量的递归模块就足够了。最后，一个线性投影层是用于产生在每个时间步上的下降方向∆x<sub>(t)</sub>。
