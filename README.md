...
#### 评估
不幸的是，没有一致的方法报告错误的人脸对齐，即使是对于常见的300W测试集。这主要是由于误差归一化的变化。为了与300W比赛[ The first facial landmark localization challenge]的结果保持一致，我们使用了他们对眼间距的定义，即外眼角之间的距离。我们认为，平均误差，特别是没有伴随标准差的平均误差，并不是一个非常有用的误差度量标准，因为它们可能会被少量非常差的拟合严重偏差。因此，我们以CED曲线的形式给出我们的评价，因为这与[ The first facial landmark localization challenge]得到的结果保持一致。我们已经从CED曲线中计算出了一些进一步的统计数据作为曲线下面积(area-under-the-curve, AUC)和每种方法的故障率(我们认为任何点对点误差大于0.08的拟合都是失败的)。我们认为，这些事人脸对齐问题更具有代表性的错误指标。我们还注意到，68点误差度量与49/51点误差度量之间存在显著差异，这是由于在拟合人脸轮廓的边界点时存在固有的困难。因此，可能的情况下，我们同时罗列了68点和49/51点的错误。
#### 实现细节
除非另有说明。否则我们的用于特征提取的网络由两个卷积层组成。每一层采用32过滤器，每个有一个3x3的内核。每个卷积层后面都有一个修正的线性单元（ReLU）和一个2X2的最大池化层。我们添加了一个跳转连接，并将来自第一卷积层中央作物的激活与第二池层的输出连接起来，以保留更多相关的本地化信息，否则使用max池化层会丢失这些信息。由于收敛所需的时间步长相对较小，我们发现使用维数为512的状态向量的递归模块就足够了。最后，一个线性投影层是用于产生在每个时间步上的下降方向∆x<sub>(t)</sub>。我们在[im](http://trigeorgis.com/mdm)提供了一个示例实现和一个预先培训的MDM模型。为了学习网络的权值，我们使用带有默认超参数的Adam[随机优化的一种方法]进行随机优化,初始学习率为0.001，每20,000次迭代的指数衰减为0.95.图像最小batch size 大小为50张。我们根据验证集选择最佳模型（300W 完全集）。对于所有的实验，我们的网络都是在使用300W训练集的3148张带有68点标记的图像和300W比赛提供的边框进行训练和测试。训练图像增强,以提供额外的训练数据通过增加单像素的高斯噪声的σ= 0.5，通过在垂直轴上的镜像，最后进行平面内的随机旋转±15◦生成均匀分布。
#### 5.1对比最新技术
我们在两个独立的实验中对比了最先进的方法。为了提供一个相关的基准，我们在300W的完全集上评估MDM。在图6中，我们提供了CED曲线与目前最先进的项目输出级联回归方法的对比，如POCR,CFSS,ERT,Intraface，Chehra和SDM.所有方法都使用相同的边框初始化，由300W提供。所有的方法都是由于可公开使用而被选择的。ERT使用DLib提供的，由300W训练集带着300W边框进行了重新训练。我们相信这个实验证明了目前可用的人脸对齐数据集正在趋于饱和，因为最近提出的三种方法几乎没有什么不同（CFSS,PO-CR,MDM）。从历史上看，由于缺乏可靠的训练数据，人脸对齐方法一直难以实现，这可能导致作者将上述测试集用作验证集(用于超参数优化)和评估集。

我们的初步实验是在300W比赛的测试集上进行的。图4为给出68点图和51点图的结果。结果显示，MDM在68点和51点错误度量方面的性能都优于其他face alignment方法，为人脸对齐问题设置了一个新的最新技术。值得注意的是，参加比赛的人对所使用的培训数据的数量没有任何限制，这进一步证明了我们的方法的有效性。
#### 5.2自我评价
在这一节中，我们进行了一些自我评价实验来探索我们的模型的行为。
#### 增加时步结果
在这里，我们看到在性能稳定之前只需要4次迭代
#### 学习特征结果
卷积特征提取更好
#### 分割
图3显示MDM能够根据头部姿势划分下降方向的空间
#### 6结论
我们提出了一种非线性统一模型——助记符下降法(MDM)，用于端到端学习非线性函数的下降方向。与现有的级联回归框架不同，MDM能够通过在下降方向学习中引入记忆的概念来为级联迭代之间的依赖关系建模。

我们在可变形目标对齐领域应用MDM.我们已经提出了第一个卷积递归架构，它能够以端到端方式进行训练，即，从原始图像像素强度到最终预测。通过使用卷积模块，我们已经说明MDM可以学习一组健壮的特性，这些特性在面部对齐方面优于手工制作的特性。此外，递归模块似乎利用过去的信息，如头部姿势，以数据驱动的方式划分下降方向的空间。最后，我们的方法在具有挑战性的300W测试集上，比目前最先进的人脸对准技术表现出色。
